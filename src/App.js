...
function App() {
  ...
  const handleButtonClick = (value) => {
    if (value === '=') {
      ...
      setExpressionValue('');
      setCalculation('');
    } else if (value === 'DANGANAN') {
      setExpressionValue('Aaron Danganan');
    } else {
  ...
  return (
    <div className='CalContainer'>
      <h1 className='Header'>Calculator of Aaron Danganan IT3A</h1>
      ...
      <div className='ButtonContainer'>
        <CalButton label="DANGANAN" onClick={handleButtonClick} />
      </div>
    </div>
  );
}
